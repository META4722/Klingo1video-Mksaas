      ...
      envVars.forEach((varName) => {
        ...
        if (line) {
          ...
          if (varName === 'NEXT_PUBLIC_BASE_URL') {
            ...
            console.log(
              `   ${varName} = ${value ? value.substring(0, 10) + '...' : '(空)'}`
            );
          }
        }
      });
...
try {
  ...
  if (fs.existsSync(urlsPath)) {
    ...
    if (
      content.includes('function getUrlWithLocale') ||
      content.includes('getUrlWithLocale')
    ) {
      console.log('   ✅ getUrlWithLocale 函数存在');

      // 检查函数实现
      const funcMatch = content.match(
        /function getUrlWithLocale[\s\S]+?return[\s\S]+?;/
      );
      if (funcMatch) {
        console.log('   函数实现:');
        funcMatch[0].split('\n').forEach((line) => {
